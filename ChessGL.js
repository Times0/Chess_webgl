...
const ChessGL = () => {
    ...
    const loadPieces = async () => {
        ...
        {
            pieces.push(await loadModel(position, white, pieceName[index], pieceNumber[index]));
            pieces.push(await loadModel(position, white, 'p', index));
            pieces.push(await loadModel(position, black, pieceName[index], pieceNumber[index], [0, Math.PI, 0]));
    ...
    const loadModel = (position, color, pieceName, pieceNumber, rotation=[0, 0, 0]) => {
        const path={'r': './ChessPieces/rook.glb', 'n': './ChessPieces/knight.glb', 'b': './ChessPieces/bishop.glb', 'q': './ChessPieces/queen.glb', 'k': './ChessPieces/king.glb', 'p': './ChessPieces/pawn.glb'};
        ...
        return new Promise((resolve) => {
            ...
            loader.load(path[pieceName], (gltf) => {
                ...
                piece.userData = { type: pieceName, color: pieceColor, number: pieceNumber};
                console.log('loaded');
    ...
    const drawChessboard = (handleChessClick) => {
        ...
        const createSquare = (x, z, color) => {
            ...
            return (
            <mesh key={`${x}-${z}`} geometry={geometry} position={position} onClick={(event) => {handleChessClick(event, position)}}>
    ...
    const findPieceByCoordinate = (pieces, position) => {        
        ...
        piece.position['x'] === position[0] &&
        piece.position['z'] === position[1]
        ));
    };
    ...
    const drawLegalCase = (legalCases) => {
        const createRing = (x, z, color) => {
          ...
          return (
            <mesh key={`${x}-${z}`} geometry={geometry} position={position} rotation={rotation}>
              <meshStandardMaterial color={color} />
            </mesh>
          );
        };
    ...
    const removePiece = (tpiece) => {
        ...
        if(tpiece.userData['color'] === 'w')
        {
            let ipos = [-5, 2.5];
            let t;
            while((t=findPieceByCoordinate(pieces, ipos)) !== undefined && t !== tpiece)
            {
                if(ipos[1] - 1 < -3.5)
                {
                    ipos[0] -= 1;
    ...
    const updatePiecePosition = (pieces, cmd, endCaseRef) => {
        ...
        pieces.forEach((piece, index) => {
            ...
            if(piece.userData['type'] === ref[position[2]*8+position[0]][1]
        ...
        ref.forEach((elem, index) => 
        {
            if(elem.length !== 0 && refMap[index] === undefined)
            {
                ...
                {
                    if(pieceMap[index2] === undefined && elem[0] === pieces[index2].userData['color'] && elem[1] === pieces[index2].userData['type'])   
                    {
                        let position = AbsoluteToChessCoord(pieces[index2].position);
                        if(position[0] >= 0 && position[0] < 8)
                        {
                            let p = ChessToAbsoluteCoord(new Vector3(index%8, 0, ((index-index%8)/8)));
    ...
    const kingLight = () => {
        if(chess.current !== undefined)
        {
            ...
            let king = pieces.find((piece) => (piece.userData['type'] === 'k' && piece.userData['color'] === color));
            ...
            <>
                <spotLight
                position={new THREE.Vector3(king.position['x'], 2, king.position['z'])}
                target={lightTarget}
                intensity={1}
    ...
    const DisplayChessGame = ({game, pieces}) =>
    {
        const handlePromote = async () => {
            ...
            if(promotingPiece.current !== undefined)
            {
                ...
                if(type === 'n' || type === 'b' || type === 'q' || type === 'r')
                {
                    ...
                    const color = promotingPiece.current.userData['color'] === 'w' ? white : black;
                    const rotation = promotingPiece.current.userData['color'] === 'w' ? [0, 0, 0] : [0, Math.PI, 0];
                    const index = findPieceIndexByCoordinate(pieces, position);
                        
                    pieces[index] = await loadModel([position['x'], 0, position['z']], color, type, 2, rotation);
        ...
        const handlePieceMove = async (game, pieces) =>
        {
            ...
            // Put down all pieces
            pieces.forEach((piece) => {if(piece.position['y'] !== -thick){piece.position['y'] = 0}});
            setLegalCases([]);
            ...
            if(startPieceRef.current !== undefined)
            {
                ...
                setLegalCases(game.getPieceMove(new Square(startPosition[2], startPosition[0])));
            }

            if(startPieceRef.current !== undefined && endCaseRef.current !== undefined)
            {
                ...
                if(startPieceRef.current.userData['type'] === 'p')
                {
                    ...
                    {
                        let moveCases = game.getPieceMove(new Square(startPosition[2], startPosition[0]))
                ...
                let r = game.move(  new Square(startPosition[2], startPosition[0]),
                                    new Square(endPosition[2], endPosition[0]), promotion)
                if(r !== null)
                { 
        ...
        const handleChessClick = (event, position) => {
            ...
            let tpiece = findPieceByCoordinate(pieces, [position['x'], position['z']]);
            ...
            else if(tpiece !== undefined)    
            {
                ...
                if(tpiece.userData['color'] === game.getTurn() && tposition[0] >= 0 && tposition[0] < 8)   
        ...
        return (
            ...
            {pieces.map((piece, index) => (
                <primitive key={index} object={piece} onClick={(event) => {handleChessClick(event, piece.position)}}/>
            ))}
